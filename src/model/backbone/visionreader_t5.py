...
class JoinT5Encoder(T5Stack):
    ...
    def forward(
        ...
        # V_L = img_feat.size(1)
        if self.model_type == 't5_ep3':
          V_L = 0
          if (ocr_info and obj_info) is not None:
              ocr_feat = self.ocr_embedding(ocr_info)
              obj_feat = self.obj_embedding(obj_info)
              inputs_embeds = torch.cat([inputs_embeds,obj_feat, ocr_feat], dim=1)
              V_L +=obj_feat.size(1)
              V_L +=ocr_feat.size(1)
          else:
              inputs_embeds = torch.cat([inputs_embeds], dim=1)
        elif self.model_type == 't5_ep2' or self.model_type == 't5_ep1':
          V_L = 0
          if (ocr_info and obj_info) is not None:
              ocr_feat = self.ocr_embedding(ocr_info)
              # obj_feat = self.obj_embedding(obj_info)
              inputs_embeds = torch.cat([inputs_embeds, ocr_feat], dim=1)
              # V_L +=obj_feat.size(1)
              V_L +=ocr_feat.size(1)
          else:
              inputs_embeds = torch.cat([inputs_embeds], dim=1)
...
class VRT5ForConditionalGeneration(T5ForConditionalGeneration):
