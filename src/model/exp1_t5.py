...
from peft import PeftModel, PeftConfig, PeftType, PrefixEncoder, get_peft_model, get_peft_config
from types import SimpleNamespace


class T5_VQA_Model_exp1(nn.Module):
    def __init__(self, config: Dict):
        super(T5_VQA_Model_exp1, self).__init__()
    ...
    def forward(self, questions: List[str], images: List[str], labels: List[str] = None):
        if self.with_image:
            ...
            ocr_obj_list = [f"{ocr['texts']} " for ocr in ocr_info]
